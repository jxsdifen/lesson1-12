...
gulp.task('htmlMin', function() {
    ...
    gulp.src(['bulid/assets/css/*.json','./*.html'])
	    .pipe(revCollector()) //执行文件内引用名的替换
	    .pipe(htmlmin(options))
	    .pipe(gulp.dest('bulid'));
});

gulp.task('product',gulpSequence('sass','minifycss','htmlMin'));

gulp.task('watch',function(){
    // gulp.watch('./assets/css/*.scss',['sass']);
    // gulp.watch('bulid/index.html',['sass']);
})

gulp.task('default',['product','watch'], function() {
    console.log('ok');
});