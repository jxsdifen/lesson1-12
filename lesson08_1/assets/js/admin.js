			...
			success: function(data){
				...
				if (data.state == 'success'){
					$.each(data.data,function(index,item){
						var $option = $('<option>').attr('value',item.id).html(item.typename);
	...
	event: function(){
		...
		$table.on('click','.btn-edit',function(){
			...
			$.each(that.news_data,function(index,item){
				if (item.id == _id){
					$('#newstitle_edit').val(item.title);
					$('#newsimg_edit').val(item.img);
					$('#newsdesc_edit').val(item.desc);
					$('#newsdate_edit').val(item.date.split(' ')[0]);
					var _type = item.type;
			...
			success: function(data){
				...
				if (data.state == 'success'){
					$.each(data.data,function(index,item){
						...
						var $td_id = $('<td>').html(item.id);
						var $td_title = $('<td>').html(item.title);
						var $td_type = $('<td>').html(item.typename);
						var $td_img = $('<td>');
						var $img = $('<img>').attr('src',item.img);
						$td_img.append($img);

						var $td_date = $('<td>').html(item.date.split(' ')[0]);
						var $td_order = $('<td>');
